...
def erroModelo():
  try:
    with open("indexes/erro/errosModelo.json") as theFile:
    ...
    logging.error("(erroModelo) - " + str(e))

def erroModeloAno():
  try:
    with open("indexes/erro/errosModeloAno.json") as theFile:
      data = json.load(theFile)
      path = "https://veiculos.fipe.org.br/api/veiculos/ConsultarAnoModelo"
      for item in data:
        codigoTabelaReferencia = item['codigoTabelaReferencia']
    ...
    logging.error("(erroModeloAno) - " + str(e))

def erroPreco():
  try:
    with open("indexes/erro/errosPrecox.json") as theFile:
      data = json.load(theFile)
      path = "https://veiculos.fipe.org.br/api/veiculos/ConsultarValorComTodosParametros"
      for item in data:
        codigoMarca = item["codigoMarca"]
        codigoModelo = item["codigoModelo"]
        codigoTabelaReferencia = item["codigoTabelaReferencia"]
        codigoTipoCombustivel = item["codigoTipoCombustivel"]
        codigoTipoVeiculo = item["codigoTipoVeiculo"]
